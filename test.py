...
# Initialize Selfie Segmentation (used in GESTURE mode)
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        # ============================================
            ...
            # Apply selfie segmentation for background removal to match YOLO training data
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            results_bg = selfie_segmentation.process(rgb_frame)
            if results_bg.segmentation_mask is not None:
                condition = np.stack((results_bg.segmentation_mask,) * 3, axis=-1) > 0.1
                bg_black = np.zeros(frame.shape, dtype=np.uint8)
                output_frame = np.where(condition, frame, bg_black)
            else:
